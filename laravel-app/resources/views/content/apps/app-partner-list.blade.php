...
@section('title', 'partner List - Pages')
        ...
        function deleteItem(id) {
            ...
            swal.fire({
                ...
                if (result.isConfirmed) {
                    let route = "{!! route('app-partner-delete', ['id' => '-1']) !!}";
...
@section('content')
    ...
    {{-- add button --}}
    <a href="/app/partner/add" class="btn btn-primary mb-3">
            ...
            <table class="datatables-ajax table">
                ...
                <tbody class="table-border-bottom-0">
                    @foreach ($partners as $partner)
                        <tr>
                            <td>{{ $partner->name }}</td>
                            <td>{{ $partner->media_id ? 'دارد' : '--' }}</td>
                            <td>{{ $partner->is_featured ? 'بله' : 'خیر' }}</td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item"
                                            href="{{ route('app-partner-preview', ['id' => $partner->id]) }}"><i
                                                class="mdi mdi-eye-outline me-1"></i>مشاهده</a>
                                        <a class="dropdown-item"
                                            href="{{ route('app-partner-edit', ['id' => $partner->id]) }}"><i
                                                class="mdi mdi-pencil-outline me-1"></i> ویرایش</a>
                                        <button class="dropdown-item text-danger"
                                            onclick="deleteItem({{ $partner->id }})"><i
